function ajaxError(jqXHR, textStatus, errorThrown, errorCallBackExt) {
	...
	case (400):
		...
		if(respInfo.resultCode && respInfo.resultCode=='token.error') {
			if(errorCallBackExt){
				errorCallBackExt(jqXHR.responseText);
			} else{
				confirmMsg('请重新登录', '提示信息',function(r){
					if (r) {
						window.top.location.href = top.ctx ;
					}
				});
			}
		} else {
...
$(function() {
    ...
    // $('.form-control+.input_date').datepicker('setDate', new Date());
